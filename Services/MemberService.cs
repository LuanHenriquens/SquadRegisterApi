...
namespace SquadRegisterApi.Services
{
  public class MemberService : IMemberService
  {
    ...
    public async Task<List<Member>> GetBySquad(int squad_id)
    {
      try
      {
        if (string.IsNullOrEmpty(squad_id.ToString()) || squad_id <= 0)
          throw new ValidationException("The squad_id property cannot be null, empty or 0.");

        return await this._context.Member.Where(c => c.squad_id == squad_id).ToListAsync();
      }
      catch (Exception ex)
      {
        throw new Exception(ex.GetInnerException());
